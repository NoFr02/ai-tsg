...
class Recommender_Euclidean(Recommender):

    def __init__(self, probes_compare: pd.DataFrame, probes_original: pd.DataFrame):
        """
        Initialize the Recommender_Euclidean class with given dataframes.
    ...
    def evaluate(
        ...
        """
        Evaluate the best matching probes based on Euclidean distance.
    ...
    def __calc_rating_total_nearest(self, probe_compare: pd.Series) -> float:
        """
        Calculate the Euclidean distance rating between the required probe and a comparison probe.

        Parameters:
        - probe_compare: A series representing a comparison probe.

        Returns:
        - The Euclidean distance rating.
        """
